...
def handle_stealth(player, skill, args, target=None):
    ...
    status_effects_engine.apply_effect(player, "concealed", 10, verbose=False)
...
def handle_meditate(player, skill, args, target=None):
    ...
    def _cleanup():
        status_effects_engine.remove_effect(player, "meditating")
        player.send_line("You break your meditation.")

    status_effects_engine.apply_effect(player, "meditating", 9999, verbose=False)
...
def handle_tiger_stance(player, skill, args, target=None):
    ...
    status_effects_engine.apply_effect(player, "tiger_stance", 60, verbose=False)
...
def handle_turtle_stance(player, skill, args, target=None):
    _clear_stances(player)
    status_effects_engine.apply_effect(player, "turtle_stance", 60, verbose=False)
...
def _clear_stances(player):
    for s in ["tiger_stance", "turtle_stance", "crane_stance", "sheathed_stance"]:
        if s in player.status_effects:
            status_effects_engine.remove_effect(player, s)

@register("quick_step", "evasive_step")
def handle_evasive_step(player, skill, args, target=None):
    status_effects_engine.apply_effect(player, "evasive_step", 60, verbose=False)
...
def handle_sneak(player, skill, args, target=None):
    status_effects_engine.apply_effect(player, "sneaking", 60, verbose=False)
...
def handle_parry(player, skill, args, target=None):
    status_effects_engine.apply_effect(player, "parrying", 4, verbose=False)
...
def handle_mark_target(player, skill, args, target=None):
    ...
    status_effects_engine.apply_effect(target, "marked", 30)
...
def handle_poison(player, skill, args, target=None):
    ...
    if not weapon:
        ...
        return None, True
    status_effects_engine.apply_effect(player, "poison_coated", 60, verbose=False)
...
def handle_sheathe(player, skill, args, target=None):
    ...
    status_effects_engine.apply_effect(player, "sheathed_stance", 9999, verbose=False)
...
def handle_third_eye(player, skill, args, target=None):
    status_effects_engine.apply_effect(player, "third_eye", 30, verbose=False)
...
def handle_generic_buffs(player, skill, args, target=None):
    ...
    status_effects_engine.apply_effect(player, effect_id, 30, verbose=False)
