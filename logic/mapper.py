...
def get_map_header(room, world):
    """Generates the header string: Room Name | Zone Name"""
    ...
    return f"{Colors.BOLD}{Colors.WHITE}{room.name}{Colors.RESET} | {Colors.CYAN}{zone_name}{Colors.RESET}"
...
def draw_grid(grid, player_room, radius, visited_rooms=None, ignore_fog=False, indent=0):
    ...
    indent: Number of spaces to indent the map.
    """
    output = []
    spacer = " " * indent
