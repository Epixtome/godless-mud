...
from logic.engines import combat_math, combat_lifecycle, combat_tags
...
def execute_attack(attacker, target, room, game, players_to_prompt, blessing=None):
    """
    ...
    if status_effects_engine.has_effect(attacker, "stalled"): return
    if "training_dummy" in getattr(attacker, 'tags', []): return

    if isinstance(attacker, Player) and not blessing:
    ...
    # 2. Tag Resolution
    attack_tags = combat_tags.resolve_attack_tags(attacker, blessing)
    ...
    damage = 0
    raw_damage = combat_math.calculate_base_damage(attacker, target, blessing)
    ...
    if isinstance(attacker, Player):
        ...
        telemetry.log_event(attacker, "COMBAT_DETAIL", {
            "target": target.name, "base": raw_damage, "defense": mit_ctx['defense'], "final": damage, "tags": list(attack_tags)
        })
    else:
        # Mob Logic
        damage = raw_damage
    ...
    if any(s in getattr(target, 'status_effects', {}) for s in status_effects_engine.CRITICAL_STATES):
    ...
    if damage > 0 or is_god:
        ...
        if isinstance(attacker, Player) and attacker.equipped_weapon and "bleed" in attacker.equipped_weapon.flags:
    ...
    if damage > 0 and hasattr(target, 'resources'):
        ...
        if Tags.CONCUSSIVE in attack_tags:
            resource_engine.modify_resource(target, Tags.HEAT, 15, source=attacker.name)
        if "weight" in attack_tags and "off_balance" in getattr(target, 'status_effects', {}):
            status_effects_engine.apply_effect(target, "prone", 3, log_event=True)

    # 12. Final HP Modification & Death
    resource_engine.modify_resource(target, "hp", -damage, source=attacker, context="Combat Hit")
    ...
    if target.hp <= 0:
        combat_lifecycle.handle_death(game, target, attacker)

    # Prompt Updates
    for entity in [attacker, target]:
        if isinstance(entity, Player): players_to_prompt.add(entity)
